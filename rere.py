...
class LoginWindow:
    def __init__(self, master, on_success):
        ...
        self.top.geometry("380x220")
        self.top.resizable(False, False)
        self.top.configure(bg="#f6f3ee")
        self.top.grab_set()
        self.top.protocol("WM_DELETE_WINDOW", self.master.destroy)

        tk.Label(self.top, text="💎 مجوهرات برطال", font=("Arial", 20, "bold"), bg="#f6f3ee", fg="#b45309").pack(pady=12)
        tk.Label(self.top, text="أدخل كلمة المرور", font=("Arial", 12, "bold"), bg="#f6f3ee").pack(pady=6)

        self.password_entry = tk.Entry(self.top, show="*", justify="center", font=("Arial", 14))
...
class JewelryApp:
    ...
    def build_ui(self):
        header_frame = tk.Frame(self.root, bg="#f6f3ee")
        ...
        self.inventory_tab = tk.Frame(self.notebook, bg="#f6f3ee")
        self.operation_tab = tk.Frame(self.notebook, bg="#f6f3ee")
        self.history_tab = tk.Frame(self.notebook, bg="#f6f3ee")
        self.receipt_tab = tk.Frame(self.notebook, bg="#f6f3ee")
    ...
    def build_summary_boxes(self):
        frame = tk.Frame(self.root, bg="#f6f3ee")
        ...
        for i, (title, var) in enumerate(items):
            box = tk.Frame(frame, bg="white", relief="solid", bd=1, padx=10, pady=8)
            box.grid(row=0, column=i, sticky="nsew", padx=3)
            frame.grid_columnconfigure(i, weight=1)
            tk.Label(box, text=title, font=("Arial", 10, "bold"), bg="white").pack()
            tk.Label(box, textvariable=var, font=("Arial", 13, "bold"), fg="#1d4ed8", bg="white").pack(pady=5)

    def build_inventory_tab(self):
        top = tk.Frame(self.inventory_tab, bg="#f6f3ee")
        top.pack(fill="x", padx=10, pady=10)

        form = tk.LabelFrame(top, text="إضافة / تعديل قطعة", font=("Arial", 12, "bold"), bg="#f6f3ee", padx=10, pady=10)
        form.pack(side="right", fill="y", padx=5)

        labels = ["الصنف", "اسم القطعة", "العيار", "الوزن (غ)", "الكمية", "سعر الشراء/غ", "سعر البيع/غ"]
        for i, label in enumerate(labels):
            tk.Label(form, text=label, bg="#f6f3ee", font=("Arial", 11, "bold")).grid(row=i, column=0, sticky="e", padx=5, pady=4)
        ...
        btn_frame = tk.Frame(form, bg="#f6f3ee")
        ...
        table_frame = tk.LabelFrame(top, text="قائمة المخزون", font=("Arial", 12, "bold"), bg="#f6f3ee")
        ...
        bottom_btns = tk.Frame(table_frame, bg="#f6f3ee")
    ...
    def build_operation_tab(self):
        wrapper = tk.Frame(self.operation_tab, bg="#f6f3ee")
        wrapper.pack(fill="both", expand=True, padx=10, pady=10)

        form = tk.LabelFrame(wrapper, text="تنفيذ عملية بيع أو شراء", font=("Arial", 12, "bold"), bg="#f6f3ee", padx=10, pady=10)
        form.pack(side="right", fill="y", padx=5)
        ...
        buttons = tk.Frame(form, bg="#f6f3ee")
        ...
        preview = tk.LabelFrame(wrapper, text="معاينة المخزون", font=("Arial", 12, "bold"), bg="#f6f3ee")
    ...
    def build_history_tab(self):
        top = tk.Frame(self.history_tab, bg="#f6f3ee")
        top.pack(fill="x", padx=10, pady=8)

        tk.Label(top, text="بحث في السجل", bg="#f6f3ee", font=("Arial", 11, "bold")).pack(side="right", padx=5)
        ...
        frame = tk.LabelFrame(self.history_tab, text="سجل العمليات", font=("Arial", 12, "bold"), bg="#f6f3ee")
    ...
    def build_receipt_tab(self):
        top = tk.Frame(self.receipt_tab, bg="#f6f3ee")
        top.pack(fill="x", padx=10, pady=8)
