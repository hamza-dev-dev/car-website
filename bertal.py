...
class LoginWindow:
    def __init__(self, master, on_success):
        ...
        tk.Frame(page, bg=GOLD_DARK, height=2).pack(fill="x", padx=140, pady=(0, 26))

        panel_border = tk.Frame(page, bg=GOLD_DARK, padx=3, pady=3)
        panel_border.pack(pady=10)

        panel = tk.Frame(panel_border, bg="#05070b", width=430, height=260)
        panel.pack()
...
class JewelryApp:
    ...
    def build_inventory_tab(self):
        ...
        form = tk.LabelFrame(top, text="إضافة / تعديل قطعة", font=("Arial", 12, "bold"), bg=BG_COLOR, fg=TEXT_LIGHT, padx=10, pady=10)
        form.pack(side="right", fill="y", padx=5)

        labels = ["الصنف", "اسم القطعة", "العيار", "الوزن (غ)", "الكمية", "سعر الشراء/غ", "سعر البيع/غ"]
        for i, label in enumerate(labels):
            tk.Label(form, text=label, bg=BG_COLOR, fg=TEXT_LIGHT, font=("Arial", 11, "bold")).grid(row=i, column=0, sticky="e", padx=5, pady=4)
        ...
        self.inv_name = tk.Entry(form, justify="right", font=("Arial", 11), bg="#1a1d25", fg=TEXT_LIGHT, insertbackground=TEXT_LIGHT)
        ...
        self.inv_weight = tk.Entry(form, justify="right", font=("Arial", 11), bg="#1a1d25", fg=TEXT_LIGHT, insertbackground=TEXT_LIGHT)
        self.inv_weight.grid(row=3, column=1, padx=5, pady=4)

        self.inv_quantity = tk.Entry(form, justify="right", font=("Arial", 11), bg="#1a1d25", fg=TEXT_LIGHT, insertbackground=TEXT_LIGHT)
        self.inv_quantity.grid(row=4, column=1, padx=5, pady=4)

        self.inv_buy = tk.Entry(form, justify="right", font=("Arial", 11), bg="#1a1d25", fg=TEXT_LIGHT, insertbackground=TEXT_LIGHT)
        self.inv_buy.grid(row=5, column=1, padx=5, pady=4)

        self.inv_sell = tk.Entry(form, justify="right", font=("Arial", 11), bg="#1a1d25", fg=TEXT_LIGHT, insertbackground=TEXT_LIGHT)
        ...
        table_frame = tk.LabelFrame(top, text="قائمة المخزون", font=("Arial", 12, "bold"), bg=BG_COLOR, fg=TEXT_LIGHT)
    ...
    def build_operation_tab(self):
        ...
        form = tk.LabelFrame(wrapper, text="تنفيذ عملية بيع أو شراء", font=("Arial", 12, "bold"), bg=BG_COLOR, fg=TEXT_LIGHT, padx=10, pady=10)
        form.pack(side="right", fill="y", padx=5)
        ...
        self.customer_entry = tk.Entry(form, justify="right", font=("Arial", 11), bg="#1a1d25", fg=TEXT_LIGHT, insertbackground=TEXT_LIGHT)
        self.customer_entry.grid(row=2, column=1, padx=5, pady=4)

        self.op_quantity = tk.Entry(form, justify="right", font=("Arial", 11), bg="#1a1d25", fg=TEXT_LIGHT, insertbackground=TEXT_LIGHT)
        self.op_quantity.grid(row=3, column=1, padx=5, pady=4)
        self.op_quantity.insert(0, "1")

        self.workmanship_entry = tk.Entry(form, justify="right", font=("Arial", 11), bg="#1a1d25", fg=TEXT_LIGHT, insertbackground=TEXT_LIGHT)
        self.workmanship_entry.grid(row=4, column=1, padx=5, pady=4)
        self.workmanship_entry.insert(0, "0")

        self.notes_entry = tk.Entry(form, justify="right", font=("Arial", 11), bg="#1a1d25", fg=TEXT_LIGHT, insertbackground=TEXT_LIGHT)
        ...
        preview = tk.LabelFrame(wrapper, text="معاينة المخزون", font=("Arial", 12, "bold"), bg=BG_COLOR, fg=TEXT_LIGHT)
    ...
    def build_history_tab(self):
        ...
        tk.Label(top, text="بحث في السجل", bg=BG_COLOR, fg=TEXT_LIGHT, font=("Arial", 11, "bold")).pack(side="right", padx=5)
        self.search_entry = tk.Entry(top, justify="right", font=("Arial", 11), width=30, bg="#1a1d25", fg=TEXT_LIGHT, insertbackground=TEXT_LIGHT)
        ...
        frame = tk.LabelFrame(self.history_tab, text="سجل العمليات", font=("Arial", 12, "bold"), bg=BG_COLOR, fg=TEXT_LIGHT)
    ...
    def build_receipt_tab(self):
        ...
        self.receipt_box = tk.Text(self.receipt_tab, font=("Courier New", 12), wrap="word", bg="#0f1117", fg=TEXT_LIGHT, insertbackground=TEXT_LIGHT)
    ...
    def edit_selected_inventory(self):
        ...
        messagebox.showinfo("تعديل", "تم تحميل بيانات القطعة. عدلها ثم اضغط حفظ التعديل")
    ...
    def execute_operation(self):
        try:
            ...
            new_qty = stock_qty - quantity if op_type == "بيع" else stock_qty + quantity
