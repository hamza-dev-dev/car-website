...
TEMPLATES = {
    "اللغة العربية": {
        ...
        "مؤشر الكفاءة": "أن يحدد الفكرة العامة والأفكار الأساسية، وأن يجيب عن أسئلة الفهم، وأن يوظف الرصيد اللغوي توظيفًا سليمًا.",
        ...
        "سير الحصة": "تمهيد - قراءة صامتة/جهرية - شرح المفردات - مناقشة الفهم - استخراج الأفكار - استثمار لغوي - تطبيق.",
    ...
    "العلوم الطبيعية": {
        ...
        "سير الحصة": "ملاحظة - فرضيات - نشاط/تجربة - تحليل - استنتاج - تثبيت.",
    ...
    "التربية الإسلامية": {
        ...
        "الوضعية الإدماجية": "وضعية تربوية يوظف فيها المتعلم قيمة أو حكمًا شرعيًا.",
...
class NotesApp:
    ...
    def build_ui(self):
        ...
        tk.Label(
            header,
            text="📘 برنامج مذكرات أساتذة المتوسط",
            font=("Arial", 24, "bold"),
            bg=BG,
            fg=GOLD,
        ).pack()
        tk.Label(
            header,
            text="واجهة احترافية مع قوالب جاهزة حسب المادة وحفظ وأرشفة للمذكرات",
            font=("Arial", 11),
            bg=BG,
            fg=MUTED,
        ).pack(pady=4)
        ...
        for i, label in enumerate(labels):
            ...
            if label == "المستوى":
                ...
                widget = tk.Entry(left, justify="right", width=38, font=("Arial", 11), bg="#f8fafc", fg="black")
        ...
        for idx, (label, height) in enumerate(multi_fields):
            ...
            text = tk.Text(left, width=38, height=height, font=("Arial", 11), wrap="word", bg="#f8fafc", fg="black")
        ...
        self.make_button(btn_frame, "تعديل", self.update_note, "#f59e0b", "black").pack(side="right", padx=4)
        self.make_button(btn_frame, "حذف", self.delete_note, DANGER).pack(side="right", padx=4)
        self.make_button(btn_frame, "معاينة", self.preview_note, ACCENT).pack(side="right", padx=4)
        self.make_button(btn_frame, "طباعة احترافية", self.print_note, "#0f766e").pack(side="right", padx=4)
        ...
        search_entry = tk.Entry(top_search, textvariable=self.search_var, justify="right", font=("Arial", 11), width=35, bg="#f8fafc", fg="black")
    ...
    def make_button(self, parent, text, command, bg, fg="white"):
        return tk.Button(parent, text=text, command=command, bg=bg, fg=fg, font=("Arial", 11, "bold"), padx=12, pady=8, relief="raised", bd=2, cursor="hand2")
    ...
    def validate_data(self, data):
        required = ["اسم الأستاذ", "المادة", "المستوى", "عنوان الدرس"]
    ...
    def update_note(self):
        if self.current_index is None:
            messagebox.showwarning("تنبيه", "اختر مذكرة من الجدول أولاً")
            return
        try:
...
<script>window.onload = () => setTimeout(() => window.print(), 300);</script>
    ...
    def preview_note(self):
        ...
        text = tk.Text(preview, wrap="word", font=("Arial", 12), bg="#f8fafc", fg="black")
