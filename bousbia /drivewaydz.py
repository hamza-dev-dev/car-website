    ...
    if count == 0:
        seed_cars = [
            (
                ...
                "SUV شبابية",
                "كروس أوفر عصرية بتصميم رياضي وتجهيزات حديثة، مناسبة للشباب والاستعمال اليومي.",
                "https://upload.wikimedia.org/wikipedia/commons/thumb/9/95/2021_Geely_Binyue_Pro_%28front%29.jpg/1280px-2021_Geely_Binyue_Pro_%28front%29.jpg",
            ...
            (
                ...
                "سيدان",
                "سيدان أنيقة واقتصادية مناسبة للاستعمال اليومي والعائلي مع تصميم عصري.",
                "https://upload.wikimedia.org/wikipedia/commons/thumb/8/85/2021_MG5_%28front%29.jpg/1280px-2021_MG5_%28front%29.jpg",
                datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            ),
            (
                "Lifan X60",
                "ابتداءً من 3,950,000 دج",
                "SUV عائلية",
                "سيارة متعددة الاستعمالات بمساحة جيدة وسعر مناسب للعائلة والتنقلات الطويلة.",
                "https://upload.wikimedia.org/wikipedia/commons/thumb/3/31/Lifan_X60_FL.jpg/1280px-Lifan_X60_FL.jpg",
                datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            ),
            (
                "Changan CS35 Plus",
                "ابتداءً من 4,890,000 دج",
                "SUV مدمجة",
                "موديل عصري من Changan مناسب للمدينة والسفر مع تجهيزات مريحة وتصميم حديث.",
                "https://upload.wikimedia.org/wikipedia/commons/thumb/d/d8/Changan_CS35_Plus_001.jpg/1280px-Changan_CS35_Plus_001.jpg",
                datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            ),
        ]
        cur.executemany(
...
<style>
    ...
    * { box-sizing: border-box; }
    ...
    @media (max-width: 1100px) {
        .hero-grid, .features-grid, .contact-grid, .cars-grid, .steps-grid, .admin-grid {
    ...
    @media (max-width: 820px) {
        ...
        .nav, .section-head, .hero-actions, .admin-top {
            ...
            <div class="cars-grid">
                ...
                <article class="car-card">
                    ...
                    <div class="car-body">
                        ...
                        <p>{{ car['description'] }}</p>
                        <div class="actions">
                            <a class="btn btn-primary" href="{{ url_for('car_details', car_id=car['id']) }}">عرض التفاصيل</a>
                            <a class="btn btn-outline" target="_blank"
                               href="https://wa.me/213777777777?text=السلام عليكم، أريد معلومات حول سيارة {{ car['name'] }}">
                               واتساب
                            </a>
                        </div>
                    </div>
...
CAR_DETAILS_TEMPLATE = """
<!DOCTYPE html>
<html lang="ar" dir="rtl">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>{{ car['name'] }} | Drivewaydz</title>
    {{ css|safe }}
</head>
<body>
    <div class="topbar">
        <div class="container nav">
            ...
            <div class="menu">
                ...
                <a href="{{ url_for('home') }}#cars">السيارات</a>
                <a href="{{ url_for('admin_login') }}" class="btn btn-outline">لوحة التحكم</a>
            </div>
        </div>
    </div>
    ...
    <section>
        <div class="container">
            <div class="panel" style="padding: 30px;">
                <div class="hero-grid">
                    <div>
                        <img src="{{ car['image'] }}" alt="{{ car['name'] }}" style="width:100%; border-radius:24px; max-height:520px; object-fit:cover;">
                    </div>
                    <div>
                        <span class="tag">{{ car['category'] }}</span>
                        <h1 style="margin:12px 0; font-size:42px;">{{ car['name'] }}</h1>
                        <div style="font-size:28px; font-weight:800; margin-bottom:18px;">{{ car['price'] }}</div>
                        <p style="color:#475569; line-height:2; font-size:18px;">{{ car['description'] }}</p>

                        <div class="actions" style="margin-top:24px;">
                            <a class="btn btn-primary" href="{{ url_for('home') }}#contact">اطلب هذه السيارة</a>
                            <a class="btn btn-outline" target="_blank"
                               href="https://wa.me/213777777777?text=السلام عليكم، أريد معلومات حول سيارة {{ car['name'] }}">
                               واتساب
                            </a>
                        ...
                        <div class="panel" style="margin-top:24px;">
                            <h3 style="margin-top:0;">معلومات إضافية</h3>
                            <ul class="admin-list">
                                <li>الاسم: <strong>{{ car['name'] }}</strong></li>
                                <li>الفئة: <strong>{{ car['category'] }}</strong></li>
                                <li>السعر: <strong>{{ car['price'] }}</strong></li>
                                <li>متوفر للطلب عبر Drivewaydz</li>
                            </ul>
                        </div>
                    </div>
                </div>
        ...
        <div class="container">
            ...
            <div class="admin-grid">
                <div class="panel" id="add-car">
                    ...
                    <form method="POST" action="{{ url_for('save_car') }}" enctype="multipart/form-data">
                        ...
                        <input type="text" name="image" placeholder="رابط الصورة (اختياري)" value="{{ edit_car['image'] if edit_car else '' }}">
                        <input type="file" name="image_file" accept="image/*">
                        <div class="help-text">يمكنك وضع رابط صورة أو رفع صورة من جهازك. إذا رفعت صورة، سيتم اعتمادها تلقائياً.</div>
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="actions">
                                    <a class="btn btn-outline" href="{{ url_for('car_details', car_id=car['id']) }}" target="_blank">عرض</a>
...
def save_car():
    ...
    final_image = image_url or "https://images.unsplash.com/photo-1492144534655-ae79c964c9d7?auto=format&fit=crop&w=1200&q=80"

    if image_file and image_file.filename:
        if allowed_file(image_file.filename):
            filename = secure_filename(image_file.filename)
            extension = filename.rsplit(".", 1)[1].lower()
            unique_name = f"{datetime.now().strftime('%Y%m%d%H%M%S')}_{name[:20].replace(' ', '_')}.{extension}"
            file_path = os.path.join(app.config["UPLOAD_FOLDER"], unique_name)
            image_file.save(file_path)
            final_image = "/" + file_path.replace("\\", "/")
        else:
            flash("صيغة الصورة غير مدعومة. استعمل png أو jpg أو jpeg أو gif أو webp.", "error")
            return redirect(url_for("admin_dashboard"))

    conn = get_db_connection()

    if car_id.isdigit():
        old_car = conn.execute("SELECT * FROM cars WHERE id = ?", (car_id,)).fetchone()
        if old_car:
            old_image = old_car["image"]
            if final_image != old_image and old_image and old_image.startswith("/static/uploads/"):
        ...
        conn.execute(
            ...
            (name, price, category, description, final_image, car_id),
        )
        flash("تم تعديل السيارة بنجاح.")
    else:
        conn.execute(
            "INSERT INTO cars (name, price, category, description, image, created_at) VALUES (?, ?, ?, ?, ?, ?)",
            (name, price, category, description, final_image, datetime.now().strftime("%Y-%m-%d %H:%M:%S")),
        )
...
def delete_car(car_id):
    ...
    if car:
        image = car["image"]
        if image and image.startswith("/static/uploads/"):
